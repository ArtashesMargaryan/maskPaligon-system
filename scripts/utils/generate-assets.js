...
function processImagesLocalized(p) {
    const directories = getDirectories(p);
    let images = '';
    const value = directories.reduce((str, directory) => {
        const keyLocalized = path.relative(p, directory);
        const filesLocalized = getFilesRecursively(directory);
        const valueLocalized = filesLocalized.reduce((str, image) => {
            const key = path.relative(directory, image);
            const value = image.replace(cwd, '..');
            str += `'${trimExt(key)}':require('${value}').default,`;
            return str;
        }, '');
        images += `'${trimExt(keyLocalized)}':'${keyLocalized}',`;
        str += `'${keyLocalized}':{${valueLocalized}},`;
        return str;
    }, '');
    return [value, images];
}
    ...
    directories.forEach((d) => {
        ...
        switch (type) {
            case 'images':
                ...
                break;
            case 'images_localized':
                {
                    const processedImages = processImagesLocalized(d);
                    assets = assets.replace(`'images_localized': {},`, `'images_localized':{${processedImages[0]}},`);
                    images += processedImages[1];
                }
                break;
            case 'sounds':
