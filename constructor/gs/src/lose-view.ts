...
const getPopupShowTweenConfig = (target: PIXI.DisplayObject): TweenConfig => {
    return {
        universal: true,
        pixi: { alpha: 0.6, scale: 0 },
        ease: PIXI.tween.easeBackOut.config(0.7),
        duration: 0.7,
        delay: 0.2,
        onStart: () => (target.visible = true),
    };
...
const getPopupHideTweenConfig = (): TweenConfig => {
    return {
        universal: true,
        pixi: { scale: 0 },
        ease: PIXI.tween.easeBackIn,
        duration: 0.6,
    };
};

const getRetryBtnHideTweenConfig = (): TweenConfig => {
    return {
        universal: true,
        pixi: { scale: 0 },
        ease: PIXI.tween.easeBackIn.config(2.4),
        duration: 0.66,
    };
};
...
export class LoseView extends ResultViewAbstract {
    ...
    private _hide(): void {
        PIXI.tween
            .timeline({ universal: true })
            .add([
                PIXI.tween.to(this._popup, getPopupHideTweenConfig()),
                PIXI.tween.to(this._retryBtn, getRetryBtnHideTweenConfig()),
            ])
